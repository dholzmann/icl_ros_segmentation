...
namespace icl {
  namespace geom {
    ...
    void FeatureGraphSegmenter::setCornerRemoval (int ll, int lr, int ul, int ur){
      m_data->llCorner=ll;
      m_data->lrCorner=lr;
      m_data->ulCorner=ul;
      m_data->urCorner=ur;
    ...
    void FeatureGraphSegmenter::surfaceSegmentation(core::DataSegment<float,4> &xyz, const core::Img8u &edgeImg, 
      ...
      if(useROI){//create mask
		  ...
		  for(int x=0; x<m_data->urCorner; x++){
		    for(int y=0; y<m_data->urCorner-x; y++){
		      m_data->maskImage(m_data->maskImage.getSize().width-x-1,y,0)=1;
		    }
		  }
		  for(int x=0; x<m_data->llCorner; x++){
		    for(int y=0; y<m_data->llCorner-x; y++){
		      m_data->maskImage(x,m_data->maskImage.getSize().height-y-1,0)=1;
		    }
		  }
		  for(int x=0; x<m_data->lrCorner; x++){
		    for(int y=0; y<m_data->lrCorner-x; y++){
		      m_data->maskImage(m_data->maskImage.getSize().width-x-1,m_data->maskImage.getSize().height-y-1,0)=1;
		    }
		  }
