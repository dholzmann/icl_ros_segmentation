...
/** @param adjacencyMatrix the input symmetric adjacency matrix (0 for "no edge", >0 for edge capacity)
    ...
    @return the cut cost.
*/
static float minCut(Mat &adjacencyMatrix, std::vector<int> &subset1, std::vector<int> &subset2);
...
*/
static std::vector<std::vector<int> > thresholdCut(Mat &adjacencyMatrix, float threshold);
...
*/
static std::vector<CutNode> hierarchicalCut(Mat &adjacencyMatrix);
...
*/
static Mat calculateProbabilityMatrix(Mat &initialMatrix, bool symmetry=true);
...
/** @param adjacencyMatrix the input matrix (0 for "no edge", >0 for edge weights)
    @return a list of subsets.
*/
static std::vector<std::vector<int> > findUnconnectedSubgraphs(Mat &adjacencyMatrix);
...
*/
    static Mat createSubMatrix(Mat &adjacencyMatrix, std::vector<int> &subgraph);

    /// Merges the src matrix into the dst matrix (dst is the maximum of both matrices).
/** @param dst the destination matrix
    @param src the source matrix
*/
    static void mergeMatrix(Mat &dst, Mat &src);
...
*/
    static void weightMatrix(Mat &dst, Mat &featureMatrix, float weight);

private:
static std::vector<float> capforest(std::vector<Point> &edgeList, std::vector<float> &edgeCosts, int subsetsSize);

static float initialLambda(Mat &adjacencyMatrix, int &lambda_id);

static void createEdgeList(Mat &adjacencyMatrix, std::vector<Point> &edgeList, std::vector<float> &edgeCosts);

static std::vector<std::vector<int> > createInitialNodes(Mat &adjacencyMatrix);
