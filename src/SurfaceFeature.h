...
class SurfaceFeatureExtract{
  ...
  public:
    ...
    */
    static SurfaceRegionFeature apply(std::vector<Vec4f> &points, std::vector<Vec4f> &normals, int mode=ALL);
    ...
    */
    static std::vector<SurfaceRegionFeature> apply(Mat labelImage, Mat &xyzh, Mat &normals, int mode=ALL);
  ...
  private:
    ...
    static SurfaceRegionFeature getInitializedStruct();
    static void update(Vec4f &normal, Vec4f &point, SurfaceRegionFeature &feature, int mode, int x=0, int y=0);
    static void finish(SurfaceRegionFeature &feature, int mode);
};
